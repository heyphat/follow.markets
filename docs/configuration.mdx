...
5. `market`: the main configuration you will need to understand and update for your system. The market consists of a few components which have their own configurations. 
    ...
    7. `trader`: when evaluator completes its jobs with a valid signal, it will send the signal to trader to place trades. Trades will carry out from placing an order to closing trades by the trader.
        1. `allowed`: global variable to disable trader. Set it to `false`, trader won't be able to trade. You can also switch this on the tele bot via the notifier after authorizing identity.
        2. `allowed_markets`: `CASH` or/and `FUTURES`. 
        3. `allowed_patterns`: if you want to trade only 1 ticker, say `BTCUSDT`, you can set it here. If you don't set anything, it won't trades. If you use the same regex as watcher, It will trade the entire markets.
        4. `min_balance_to_trade`: the amount in dollar to place an order. With the `FUTURES` market, this amount will be multiply by `max_leverage`. If you don't have enough this minimum amount of balance on the quote currency, it won't trade.
        ...
        6. `max_wait_to_fill`: the trader will cancel the order if it won't be matched after this amount of seconds.
        7. `loss_tolerance`: the loss tolerance per trade based on the current best price and average filled price.
        8. `profit_margin`: the profit margin per trade based on the current best price and average filled price.
