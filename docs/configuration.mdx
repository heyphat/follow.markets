...
5. `market`: the main configuration you will need to understand and update for your system. The market consists of a few components which have their own configurations. 
    ...
    7. `trader`: when evaluator completes its jobs with a valid signal, it will send the signal to trader to place trades. Trades will be carried out from placing a limit order (always limit order set by the signal) to converting base currency back to quote currency by the trader.
        1. `allowed`: global variable to disable trader. Set it to `false`, trader won't be able to trade. You can also switch this on the tele bot via the notifier after authorizing identity or update the trader's configuration via trader APIs.
        2. `allowed_markets`: `CASH` or/and `FUTURES`. 
        3. `allowed_patterns`: if you want to trade only 1 ticker, say `BTCUSDT`, you can set it here. If you don't set anything, it won't trades. If you use the same regex as the watcher, It will trade the entire markets.
