...
1. `env`: the targeted enviroment when deploying the application, `development` or `production`. If it is `development`, some parts of the application won't be fully initialized to save API call credit. For example, `coinmarketcap` API calls.
...
5. `market`: the main configuration you will need to understand and update for your system. The market consists of a few components which have their own configurations. 
    ...
    2. `notifier`: the application uses telegram as a communication tool with users. Telegram is free and easy to use. 
        1. `bot_token`: a telegram bot token. Ask the [BotFather](https://core.telegram.org/bots) for a telegram `bot_token` if you don't know how to get it yet. Then start a conversation with your bot after deploying the system. The bot'll notify you messages when your signals are sucessfully evaluated.
    ...
    3. `watcher`: is responsible for watching the market. It holds the market data of all runners on the watchlist. 
        1. `base_market`: usually `USDT`, if you want `BUSD`, you can set it here. 
        2. `watchlist`: a list of regex patterns that are matched against all tradable tickers on Binance spot and futures markets. The sample watchlist in `configs/configs.json` matches all `USDT`-based pairs.
    ...
    4. `evaluator`: is responsible for evaluating your signals. Refer to the `evaluator` docs for more information about how to build a signal. The values should only be changed if you know what you are doing. 
        1. `singal.source_path`: the place to store all the signals. 
    5. `tester`: is responsible for testing your strategies. Refer to the `tester` docs for more information about how to execute a backtest. The values should only be changed if you know what you are doing.
